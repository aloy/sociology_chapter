...
Residuals are central to the exploration of a LME model, similar to the classical linear model with uncorrelated error structure. For LME models residual analysis is complicated by the fact that there are numerous quantities that can be defined as \emph{residuals}, with each residual quantity being associated with different aspects of the model. The two fundamental residuals for model checking considered here are:
%
\begin{itemize}
...
\begin{figure}
	...
	\caption{\label{fig:fanned} One of the lineups used in an Amazon Turk study. Participants were asked  to identify the most `different' plot and give a reason for their choice. 11 of the 73 observers identified the data plot as the most different. Further details on data, model and the location of the data plot are given in section~\ref{sec:random}.
	}
\end{figure}
...
\begin{figure}[h]
	...
	\caption{\label{fig:boxplot-ordered} {Lineup of level-1 residuals in side-by-side box plots to test significance of a discrete covariate. 65 out of 74 participants identified the data plot from the lineup. }}
...
\begin{figure}
	...
	\caption{\label{fig:fanned2} A follow-up to Figure~\ref{fig:fanned} where the random slope has been included in the model from which the null plots are created. None of the 64 observers identified the data plot as the most different. }
\end{figure}
...
%	Lineup of 20 boxplots (ordered by IQR) of level-1 residuals used to test the assumption of homogeneous level-1 residual variance.  Which is the real plot?
 50 out of 75 observers chose the data plot as the most different, indicating a significant deviation from the assumptions of the residual distribution in a model of the observed data.} }
\end{figure}
