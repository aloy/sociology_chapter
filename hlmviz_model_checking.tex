...
While the use of statistical graphics is wide-spread in the exploratory data analysis  and for informal validation, graphics are often considered to be too subjective as the sole basis for model selection or validation. 
At the core of this criticism is the fact that decisions are being made based on a single plot. When a single plot is used, the detection of any unexpected structure leads to the conclusion that the model is inappropriate; however, \hh{complex models introduce structures  that are then observed e.g.~} 
%artificial structures are often observed 
in the residuals of properly specified complex models \citep[c.f.,][]{Morrell:2000ve, Gelman:2000dr}. 
...
while
the alternative hypothesis encompasses any violation of this model assumption. For visual inference, the test statistic corresponds to a plot  that displays an aspect of  the model assumption which allows the observer to distinguish between scenarios under the null from scenarios under alternate hypotheses. 
...
Linear mixed-effects models are versatile models that allow to account for dependence structure when data are organized in groups.
...
Some of the problems which we are addressing in this paper are:
\begin{description}
...
\item[Artifact due to data structure: ] Residual plots might display noticeable patterns that are artifacts of the model estimation procedure rather than indications of lack of fit. 
This problem is especially pronounced for plots of the predicted random effects \cite[c.f.,][]{Morrell:2000ve}.\todo[inline]{what kind of patterns does Morrell refer to? - Let's be more specific here, or any patterns will be automatically contributed to this paper.}
...
\includegraphics[width=0.05\textwidth]{examuncorr-icon}&   fig.~\ref{fig:constvar1} & 8/77&\hspace{-0.1in}*  & 11/42&\hspace{-0.1in}*** & 22/70&\hspace{-0.1in}*** & 25/71&\hspace{-0.1in}*** & 3/67& \\ 
