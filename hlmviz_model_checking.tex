...
\citep[][introduced visual $p$-values. Details of the calculation for this LME model application are given in Section~\ref{supp::} of the supplement.]{mahbub:2013}
...
In this setting, the null plots are generated using the parametric bootstrap  with a model that omits the variable of interest, \al{details of which can be found in Section~\ref{supp::} of the supplement}. The true plot is constructed from the same model, but is fit to the observed data. 
...
The lineup in Figure~\ref{fig:fanned} was chosen to test the relationship between scores from the General Certificate of Secondary Education Exam (GCSEE) and the  standardized London Reading Test (LRT) (see Section~\ref{supp:} of the supplement for details).  Each line segment represents one of 65 inner-London schools. The slope of each line is determined by a linear regression relating the two test scores for each student at a school. 
...
\al{Figures~13 and~14 in Section~\ref{supp} of the supplement show the resulting lineups.}
