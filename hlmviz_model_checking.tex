...
\if0\blind
{
  \title{\bf  \dc{Model Choice and Diagnostics for Linear Mixed-Effects Models Using Statistics on Street Corners}}
...
there is a problem with the model. Graphical diagnostics enable the analyst to detect not only when there is a problem with the model and where it occurs, but also give some indication  %and 
...
\item {\em Model checking:} As show in Figure~\ref{fig:motivation}, residual plots might display noticeable patterns that are artifacts of both the data structure and the model estimation procedure rather than indications of lack of fit. 
...
In this setting, the null plots are generated using the parametric bootstrap  with a model that omits the variable of interest, details of which can be found in Section~\ref{app:nullplots} of the supplement. The true plot is constructed from the same model, but is fit to the observed data. 
...
The visual test is not overly sensitive to group sizes. Counties with less than 5 observations were eliminated because box plots are not appropriate for such small group sizes, but could be included in the representation as dot plots. While we are still slightly constrained by group size, we are far less constrained than with the conventional test, and have a clear way to choose the minimum group size based on our ability to render box plots.
...
\begin{figure}
	...
	\caption{\label{fig:linearity} Lineup testing  for nonlinearity of a covariate. Level-1 residuals (on $y$) from a model of the dialyzer data are plotted against pressure settings (on $x$). Pressure is clearly identified (by 60 out of 63 observers) as an important higher-order covariate of the model.  } %Which of the plots is the most different? Which feature led you to your choice?}
