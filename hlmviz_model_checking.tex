...
At the core of this criticism is the fact that decisions are being made based on a single plot. When a single plot is used, the detection of any unexpected structure leads to the conclusion that the model is inappropriate; however, complex models introduce structures  that are  observed, for example, 
%artificial structures are often observed 
in the residuals of properly specified models \citep[c.f.,][]{Morrell:2000ve, Gelman:2000dr}. 
...
(i) formulating  null and alternative hypotheses,
	(ii) calculating a test statistic \hh{of the observed data},
	(iii) comparing the test statistic to a reference (null) distribution,
	(iv) and \hh{deriving} a $p$-value on which we base our conclusion.
Each of these steps has a direct analog in visual inference, as outlined by \cite{Buja:2009hp}. As we will apply visual inference in the setting of \hh{linear mixed-effects models}, we want to highlight  parallels there. Assume that  the question of interest involves  some assumption about a model, e.g.~let us assume that the null hypothesis is  homogeneity of residual variance,
while
...
\hh{For the lineup protocol a sample of the null distribution is taken and shown in a set of null plots, in which}
the true plot is randomly embedded.  

\hh{An assessment of the lineup by human observers then  allows for the estimation of a $p$-value associated with the lineup.}
Let $x$ be the number of observers, out of $N$, who chose the data plot from the lineup. The $p$-value is then the probability that   $x$ or more observers choose the data plot, given that the data plot is not any different from the other plots in the lineup, \hh{i.e.~the null hypothesis is true}. Under the null hypothesis the probability for choosing the true plot is $1/m$, if there are a total of  $m$ plots  in the lineup. Let $X$ be the random variable describing  the number  of observers choosing the data plot. 
Under the null hypothesis, $X$ is distributed according to a \hh{distribution similar to a} Binomial distribution, \hh{but adjusted for the dependency inherent to the application of lineups in a human subject study.} 
...
%\alnote{I took out the reference to our example here and just cited your other paper instead.}
%%An example of this is given in Section~\ref{sec:qqplot}, where we compare three variations of the familiar Quantile-Quantile (Q-Q) plot \citep{Wilk:1968}.
...
Such structures occur, for example, when individuals are naturally grouped by organization (e.g.~students within schools), geography (e.g.~voters within states), or design (e.g.~respondents assigned to  interviewers). The additional flexibility offered by these models \hh{enable us} to incorporate data at the observation-level and the group-level while \hh{also accommodating} for dependencies between individuals within the same group. 

\hh{For data organized into $g$ groups, }
let us  consider a continuous response linear mixed-effects model for group $i$, $i=1, \ldots, g$:
%
...
\end{equation}
%
where $\bm{y}_i$ is the vector of outcomes for the $n_i$ level-1 units in group $i$, $\bm{X}_i$ and $\bm{Z}_i$ are design matrices for the fixed and random effects, respectively, $\bm{\beta}$ is a vector of fixed effects governing the global mean structure, $\bm{b}_i$ is a vector of random effects \hh{describing} the between-group covariance structure, and $\bm{\varepsilon}_i$ is a vector of level-1 error terms \hh{accounting for} the within-group covariance structure. We further assume that the random effects are a random sample from $\mathcal{N}(\bm{0},\ \bm{D})$ and are independent from the level-1 error terms, for which we assume a distribution of $\mathcal{N}(\bm{0},\sigma^2 \bm{I})$. %\bm{R}_i)$. 
...
As with the classical linear model with uncorrelated errors, residuals are central to the exploration of a linear mixed-effects model. For linear mixed-effects models, residual analysis is complicated by the fact that there are numerous quantities that can be defined as \emph{residuals}, with each residual quantity being associated with different aspects of the model. The two fundamental residuals for model checking \hh{that we consider here are:}
%
\begin{itemize}
...
\end{itemize}
%
where, assuming $\bm{V}$ is known, $\widehat{\bm{\beta}}$ is an estimate of the fixed effects,
...
\hh{The additional flexibility of random effects models comes at the cost of} complicating model exploration and validation. %For example:
...
\item[\al{Breakdown of } asymptotic results: ] Test statistics used for model selection and validation rely on asymptotic reference distributions which often perform poorly in finite sample situations. 
\al{For example, the empirical distribution of the predicted random effects  does not resemble the theoretical distribution unless strong assumptions are met \citep[Theorem 3.2 and Lemma 3.1]{Jiang:1998vt}. In many finite sample situations these assumptions are not met, rendering conventional use of Quantile-Quantile (Q-Q) plots and tests of the empirical distribution function ineffective for distributional assessment.}
...
\al{This problem is especially pronounced for \al{scatterplots} comparing the predicted random effects. Such plots can display points lying along a line that is oriented in conflict with the estimated correlation between the random effects \cite{Morrell:2000ve}. Additionally, we have found plots of the error terms often exhibit patterns that appear to be indicative of heteroscedasticity, but are merely consequences of data imbalances or sparsity.}


\item[Boundary issues the in comparison of nested models: ] 
\al{When evaluating the significance of terms in the random effects structure, the likelihood ratio test statistic does not have the usual $\chi^2$ reference distribution if we are testing whether a variance component lies on the boundary of the parameter space. This results in tests for the random effects that tend to be conservative.}
...
\hh{These issues are well-known, and in special circumstances} \al{adjustments to the methodology have been proposed. For example, \cite{Stram:1994wd} suggest using a 50:50 mixture of $\chi^2_q$ and $\chi^2_{q+1}$ when testing $q$ versus $q + 1$ random effects; however, this adjustment is not successful in all cases. Additionally, \cite{Lange:1989uu} suggest using weighted Q-Q plots to assess the distributional assumptions made on the random effects. This approach is effective in settings where the residual variance, $\sigma^2$, is small relative to the variance components for the random effects, but breaks down when this is not the case.} 
\hh{A different approach is to tap into the (orthogonal) power of graphical diagnostics and, together with the rigor of the visual inference framework, construct tests that address some of these problems without any further assumptions. }
...
The purpose of this paper is to discuss visual inference in the framework of linear mixed-effects models and suggest  graphical tests in situations where the assumptions of conventional inference are violated. 
...
\item[\bf Random components: ] When the covariance parameter being tested lies on the boundary of the parameter space the asymptotic distribution of the likelihood ratio is no longer a $\chi^2$ distribution. Approximations have been suggested and shown to be useful in many situations \citep{Stram:1994wd, Morrell:1998ua}, but no one approximation holds for all situations. \al{This leads to a need for simulation studies to determine the proper adjustment to the reference distribution in every situation, or utilize the rule of thumb suggested by \citeauthor{Stram:1994wd} with the knowledge that the results may be sub-optimal.}

 leading to either conservative or anti-conservative decisions. 
\end{description}
...
%Revisiting Figure~\ref{fig:fanned} we see an example of testing for the inclusion of a random effect. Recall that the initial model was a random intercept model using a student's score on the standardized London Reading Test to describe their age 16 score on the General Certificate of Secondary Education examination.
 ...
 If the model is appropriate, then the overall pattern of the lines in the null plots should resemble the observed data. In this example, we find that the true plot (panel $\sqrt{144} + 4$) is identifiable, \hh{as 11 of the 73 observers pick the data plot. The main comments for their choice were the spread and trend of the line segments in the plot. This is consistent with a larger variance of slopes than the null model allows;}
...
\begin{figure}
	\centering
	\includegraphics[width = \textwidth]{normexam_fanned_withslope_lineup18}
...
%Note that with a higher amount of shrinkage in the random effects we start to see more lines with a higher amount of slope. 
 ...
 \hh{The lineup allows us to gauge the amount of correlation between the random effects while accounting for the effect of shrinkage in the model,} \al{avoiding the over-interpretation of structure in such plots discussed by \cite{Morrell:2000ve}.}
  In Figure~\ref{fig:ranef-corr} the true plot (panel~$10 + \sqrt{25}$) was identified \al{by 41 of the 69 observers}, providing very strong evidence supporting the inclusion of the additional parameter.
...
\begin{figure}
	\centering
	\includegraphics[width=.9\textwidth]{dialyzerhomogeneous-dots-1-19.pdf}
...
\begin{figure}
	\centering
	\includegraphics[width=.9\textwidth]{dialyzerhomogeneous-1-19.pdf}
...
\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{dialyzerheterogeneous-19.pdf}
...
\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{dialyzerheterogeneous-bp-19.pdf}
	\caption{\label{fig:constvar2.bp} 
	\hh{Alternative box plot } Lineup testing homogeneity of the level-1 residuals. Which of the plots is the most different? Which feature led you to your choice?}
%	Lineup of 20 scatterplots of level-1 residuals against pressure used to test the assumption of homogeneous level-1 residual variance for the dialyzer study.  Which is the real plot?}
\end{figure}
...
\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{cyclone-13.pdf}
...
\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{radon_cyclone10.pdf}
...
\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{dialyzernonlinear-10.pdf}
...
\al{To extend checks of linearity to group-level variables we suggest the use of the level-2 residuals. Additionally, when categorical predictors are investigated, side-by-side boxplots should be used.
}
...
\hhnote{we can make this an even stronger statement. Cite your JCGS submission - by the time we're through any review process, this will be resolved.}
...
To determine whether lineups can detect distributional violations we constructed another lineup where the ``true plot'' was constructed from random effects simulated from a $t_3$ distrubution. This lineup is show in Figure~\ref{fig:qqlineup-t}. \al{Twenty nine out of 70} identified the true plot (panel $\sqrt{49} + 3\cdot4$), providing evidence against the null hypothesis of normality.  
...
\hhnote{ I think we should include both a positive and a negative result for the Q-Q plot tests for normality - I am including five sets each of random intercepts and random slopes for the radon study. }
\begin{figure}
	\centering
%	\includegraphics[width=\textwidth]{qqplot_normranef_slope_lineup11.pdf}
	...
	Lineup testing  for normality of the random slope for the radon data. Which of the plots is the most different? Which feature led you to your choice? \hh{***LINEUP*** we wanted to have five nulls to make this fit into the general framework.}}
\end{figure}

\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{radontranef-19.pdf}
	\caption{\label{fig:qqlineup-t} Lineup  testing for normality of random slopes \al{simulated from a $t_3$ distribution}. Which of the plots is the most different? Which feature led you to your choice?}
\end{figure}
...
\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{autism-fanned2-16.pdf}
...
\begin{table}[ht]
\centering
\caption{\label{tab:results} Overview of all lineup evaluations. Ratios comparing the number correct to the total number of evaluations are shown. Significance was determined from the visual $p$-value defined in Equation~\ref{eqn.pvalue}. } 
\begin{tabular}{Mrrlrlrlrlrl}
  \hline
&  & \multicolumn{9}{c}{Replicate} \\ \cline{3-12}
& Lineup & 1 & & 2 && 3 && 4 && 5 \\ 
  \hline
  
\includegraphics[width=0.05\textwidth]{examfanned-icon}&  fig.~\ref{fig:fanned} & 11/73&\hspace{-0.1in}* & 7/70&\hspace{-0.1in} & 9/65&\hspace{-0.1in}*& 13/66&\hspace{-0.1in}** & 7/72&\hspace{-0.1in} \\ 
\includegraphics[width=0.05\textwidth]{autism-ordered-icon}&   fig.~\ref{fig:boxplot-ordered} & 65/74&\hspace{-0.1in}*** & 57/66&\hspace{-0.1in}*** & 62/68&\hspace{-0.1in}*** & 54/63&\hspace{-0.1in}*** & 66/76&\hspace{-0.1in}*** \\ 
\includegraphics[width=0.05\textwidth]{examcorr-icon}&   fig.~\ref{fig:ranef-corr} & 41/69&\hspace{-0.1in}*** & 62/73&\hspace{-0.1in}*** & 31/64&\hspace{-0.1in}*** & 57/75&\hspace{-0.1in}*** & 55/67&\hspace{-0.1in}*** \\ 
\includegraphics[width=0.05\textwidth]{dialyzerheterogeneous-icon}& fig.~\ref{fig:constvar2} & 29/85&\hspace{-0.1in}***  & 10/65&\hspace{-0.1in}* & 24/64&\hspace{-0.1in}*** & 7/60&\hspace{-0.1in}. & 13/72&\hspace{-0.1in}** \\ 
\includegraphics[width=0.05\textwidth]{dialyzerheterogeneous-bp} &  fig.~\ref{fig:constvar2.bp} & 23/70 & \hspace{-0.1in}*** & 9/74 & \hspace{-0.1in}. & 11/62 & \hspace{-0.1in}** & 31/78 & \hspace{-0.1in}*** & 25/61 & \hspace{-0.1in}*** \\ 
\includegraphics[width=0.05\textwidth]{cyclone-icon}&   fig.~\ref{fig:badcyclone} & 50/75&\hspace{-0.1in}***  & 44/64&\hspace{-0.1in}*** & 45/68&\hspace{-0.1in}*** & 46/76&\hspace{-0.1in}*** & 50/67&\hspace{-0.1in}*** \\ 
\includegraphics[width=0.05\textwidth]{cyclone-good-icon} & fig.~\ref{fig:goodcyclone}  & 1/59 & \hspace{-0.1in}  & 2/79 & \hspace{-0.1in}  & 2/68 & \hspace{-0.1in}  & 4/62 & \hspace{-0.1in}  & 1/73 & \hspace{-0.1in}  \\ 
\includegraphics[width=0.05\textwidth]{dialyzernonlinear-icon}&   fig.~\ref{fig:linearity} & 60/63&\hspace{-0.1in}***  & 62/64&\hspace{-0.1in}*** & 57/60&\hspace{-0.1in}*** & 84/88&\hspace{-0.1in}*** & 66/69&\hspace{-0.1in}*** \\ 
\includegraphics[width=0.05\textwidth]{radontranef-icon}&   fig.~\ref{fig:qqlineup-t} & 29/70&\hspace{-0.1in}***  & 52/79&\hspace{-0.1in}*** & 0/64& & 13/59&\hspace{-0.1in}*** & 6/74& \\ 
   & random intercept & 0/72 & \hspace{-0.1in}  & 1/75 & \hspace{-0.1in}  & 0/68 & \hspace{-0.1in}  & 0/75 & \hspace{-0.1in}  & 2/61 & \hspace{-0.1in}  \\ 
   & random slope & 0/65 & \hspace{-0.1in}  & 0/64 & \hspace{-0.1in}  & 0/68 & \hspace{-0.1in}  & 0/46 & \hspace{-0.1in}  & 0/64 & \hspace{-0.1in}  \\ 
\includegraphics[width=0.05\textwidth]{autism2-fanned-icon}&   fig.~\ref{fig:autism-ranef} & 50/79&\hspace{-0.1in}***  & 26/59&\hspace{-0.1in}*** & 31/67&\hspace{-0.1in}*** & 32/72&\hspace{-0.1in}*** & 35/71&\hspace{-0.1in}*** \\ 
\includegraphics[width=0.05\textwidth]{autism-unordered-icon}&   fig.~\ref{fig:boxplot-unordered} & 52/54&\hspace{-0.1in}***  & 63/76&\hspace{-0.1in}*** & 70/75&\hspace{-0.1in}*** & 59/64&\hspace{-0.1in}*** & 63/70&\hspace{-0.1in}*** \\ 
 \includegraphics[width=0.05\textwidth]{homogeneous-bp-icon} &  fig.~\ref{homogeneous-2}& 23/61 & \hspace{-0.1in}*** & 38/72 & \hspace{-0.1in}*** & 29/59 & \hspace{-0.1in}*** & 22/70 & \hspace{-0.1in}*** & 35/62 & \hspace{-0.1in}*** \\ 
 \includegraphics[width=0.05\textwidth]{homogeneous-dots-icon} & fig.~\ref{homogeneous-1}& 0/57 & \hspace{-0.1in}  & 1/71 & \hspace{-0.1in}  & 8/75 & \hspace{-0.1in}  & 0/59 & \hspace{-0.1in}  & 2/68 & \hspace{-0.1in}  \\ 
 \includegraphics[width=0.05\textwidth]{exam-with-slope-icon} & fig.~\ref{fig:fanned2} & 0/64 & \hspace{-0.1in}  & 7/75 & \hspace{-0.1in}  & 11/76 & \hspace{-0.1in}* & 0/69 & \hspace{-0.1in}  & 0/60 & \hspace{-0.1in}  \\ 
   \hline
\multicolumn{8}{l}{Signif. codes:  0 Ô***Õ 0.001 Ô**Õ 0.01 Ô*Õ 0.05 Ô.Õ 0.1 Ô Õ 1} \\
