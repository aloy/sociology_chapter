...
=======
where $\bm{y}_i$ is the vector of outcomes for the $n_i$ level-1 units in group $i$, $\bm{X}_i$ and $\bm{Z}_i$ are design matrices for the $p$ fixed and $q$ random effects, respectively, $\bm{\beta}$ is a vector of fixed effects governing the global mean structure, $\bm{b}_i$ is a vector of random effects \hh{describing} the between-group covariance structure, and $\bm{\varepsilon}_i$ is a vector of level-1 error terms \hh{accounting for} the within-group covariance structure. We further assume that the random effects are distributed according to  $\mathcal{N}(\bm{0},\ \bm{D})$ and are independent from the level-1 error terms, for which we assume a distribution of $\mathcal{N}(\bm{0},\sigma^2 \bm{I})$. %\bm{R}_i)$. 
...
\begin{description}
\item[Asymptotic results for finite samples: ] Test statistics used for model selection and validation rely on asymptotic reference distributions which often perform poorly in finite sample situations. 
...
The empirical distribution of the random effects often does not resemble the theoretical distribution rendering conventional use of normality tests and Quantile-Quantile plots ineffective for distributional assessment.
