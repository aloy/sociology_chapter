...
\al{Model checking is an essential step of statistical modeling to ensure the  assumptions necessary for valid inference are upheld. This process includes both the search for contradictions of model assumptions and the assessment of how well the model captures the characteristics of the data. Such an investigation can be carried out using test statistics and $p$-values to gauge the strength of evidence, however, such methods indicate only that there is a problem with the model. In contrast, graphical diagnostics enable the analyst to detect when there is a problem with the model and what may be the cause of the problem. For complex models the insight provided by graphical diagnostics is especially useful, allowing the analyst to develop intuition and make discoveries about the nature of model violations which would be nearly impossible through purely numeric methods. 
...
\al{
Based on our discussions with other statisticians, we have found that all four plots could be deemed indicative of a violation; however, only the plot of the residuals vs. the predictor is truly problematic.  Figure~\ref{fig:motivation} demonstrates that using only a single plot to distinguish between structure introduced by deviations from the model and structure introduced by the model fitting procedure is difficult, especially for more complicated statistical models such as LMEs. The purpose of this paper is to illustrate how visual inference allows us to distinguish between structure introduced by deviations from the model and structure introduced by the model fitting procedure. Additionally, the proposed graphical tests can be used in situations where the assumptions of conventional inferential procedures are violated. While we focus only on LMEs the discussion is general enough to illustrate how visual inference can be used to overcome similar issues in other model classes. 

\alnote{Do we need to give an example above?}
...
\begin{table}[ht]
\caption{\label{tab:pvalues} A comparison of the naive and the bootstrap version of the conventional test by Raudenbush and Bryk of homoscedastic error terms across groups for the radon data. \al{The naive $p$-value refers to the $p$-value obtained using the $\chi^2$ approximation. The simulated $p$-values were calculated from the empirical distribution of the $H$-statistic based on 10,000 simulated statistics obtained using the parametric bootstrap for each minimum group size.} There is a clear discrepancy between the two $p$-values indicating a need for simulation-based methods. Additionally, the sensitivity of the simulation-based test to the minimum group size is apparent.} 
\centering
