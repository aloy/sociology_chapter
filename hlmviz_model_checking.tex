...
Figure~\ref{fig:boxplot-ordered} illustrates the use of this type of lineup. In our study, 74 observers were asked to choose the plot that is the most different from the rest. Sixty five observers identified the data plot in panel \#($2^3 + 2$)\footnote{The reason for writing the panel which shows the data encoded in a bit of calculus, is meant to pose a cognitive obstacle that allows the reader to evaluate the lineup before previously being biased by knowing the answer.}, 
