...
\begin{description}
\item[\bf Fixed effects: ] Likelihood ratio tests based on REML estimation cannot be used to test different fixed effects structures. Maximum likelihood estimation allows for such comparisons, but is anti-conservative. \al{Additionally, if one uses $t$-tests or $F$-tests rather than likelihood ratio tests a different complication is encountered, defining the degrees of freedom. }
