...
\begin{figure}[!h]
	...
	\caption{\label{fig:motivation} \dc{Four residual plots from different LME models. Which ones, or all, of these would you consider to indicate structure not captured by the model?}
	}
\end{figure}
...
\alnote{If we are trying to save space, then we might be able to shorten the above point, since section 1 illustrated this issue.}
...
\begin{enumerate}
\item {\bf Create lineup data:} Assuming that the proposed model (for diagnostics) or reduced model (for selection) are correct, we use a parametric bootstrap to simulate new responses, refit the model to these simulated responses, and extract the residuals of interest from the proposed model. For each lineup, this process is used to obtain $m-1=19$ simulated data sets.

\item {\bf Render lineups:} Append the observed data set and randomly assign ID numbers to the data sets between 1 and 20. These IDs are used to facet the lineup. Store the ID number for the observed data for later reference.
...
\alnote{I'm not sure how far we want to go with this. Heike, can you take this point since you know the procedure best?}

\al{XXX I'm still trying to fit all the pieces together... this paragraph is still rough...
...
\alnote{I'm not sure where the following sentence fits now that this isn't in the appendix. Do we move this to the discussion?}
