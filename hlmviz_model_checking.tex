...
\if0\blind
{
  \title{\bf  \dc{Model Choice and Diagnostics Using Statistics on Street Corners}}
...
\if1\blind
{
  ...
  \bigskip
  \begin{center}
    {\LARGE\bf  Model Choice and Diagnostics Using Statistics on Street Corners}
\end{center}
  \medskip
} \fi
...
have been proposed, but it typically requires that the analyst keeps track of all the special circumstances that may arise. In this paper we illustrate a simpler but generally applicable approach to diagnosing linear mixed effects models. We explain how to use new visual inference methods for these purposes. The approach
...
\al{Model checking is an essential step of statistical modeling to ensure the  assumptions necessary for valid inference are upheld. This process includes both the search for contradictions of model assumptions and an assessment of how well the model captures the characteristics of the data. Such investigation can be carried out using test statistics and $p$-values to gauge the strength of evidence, however, such methods indicate only \hh{the degree to which} %that 
there is a problem with the model. Graphical diagnostics enable the analyst to detect \hh{not only} when there is a problem with the model \hh{and where it occurs, but also give some indication}  %and 
what may be the cause of the problem. For complex models the insight provided by graphical diagnostics is especially useful, allowing the analyst to develop intuition and make discoveries about the nature of model violations which would be nearly impossible through purely numeric methods. 
...
\begin{figure}[!h]
	...
	\caption{\label{fig:motivation} \dc{Four residual plots from different LME models. Which ones of these would you consider to indicate structure not captured by the model?} %(We need something like this right at the start.)}
	}
\end{figure}

\al{
Based on our discussions with other statisticians, we have found that all four plots could be deemed indicative of a violation; however, only the plot of the residuals vs.\ the predictor is truly problematic.  Figure~\ref{fig:motivation} demonstrates that using only a single plot to distinguish between structure introduced by deviations from the model and structure introduced by the model fitting procedure is difficult, especially for more complicated statistical models such as LMEs. The purpose of this paper is to illustrate how visual inference allows us to distinguish between structure due to deviations from the model and structure introduced by the model fitting procedure. Additionally, the proposed graphical tests can also be used in situations where the assumptions of conventional inferential procedures are violated. While we focus only on LMEs the discussion is general enough to illustrate how visual inference can be used to overcome similar issues in other model classes. 

The remainder of this paper is organized as follows.  
Section~\ref{sec:vi} provides an introduction to the framework of visual inference and the lineup protocol. 
Section~\ref{sec:lme}...
 Sections~\ref{sec:select} and \ref{sec:checking} expand on the problems encountered in model selection and model checking, respectively, and presents solutions utilizing visual inference. 
...
In the following sections we expand on the problems encountered in model selection and model checking, respectively, and presents solutions utilizing visual inference. 
}
...
\alnote{We're running up against the page limit... can this be a supplement, or do we need to push something else out?}
