...
While the use of statistical graphics is wide-spread in exploratory data analysis and informal validation, graphics are often considered to be too subjective as the sole basis for model selection or validation. 
At the core of this criticism is the fact that decisions are being made based on a single plot. When a single plot is used, the detection of any unexpected structure leads to the conclusion that the model is inappropriate; however, \hh{complex models introduce structures  that are  observed, e.g.,~} 
...
This additional information helps us to assess which part of the null hypothesis seems to be violated, something that we are not able to extract from  classical hypothesis tests. 
...
\item[Asymptotic results: ] Test statistics used for model selection and validation rely on asymptotic reference distributions which often perform poorly in finite sample situations. 
\al{For example, the empirical distribution of the predicted random effects  does not resemble the theoretical distribution unless strong assumptions are met \citep[Theorem 3.2 and Lemma 3.1]{Jiang:1998vt}. In finite sample situations these assumptions are often not met, rendering the conventional use of Q-Q plots and tests of the empirical distribution function ineffective for distributional assessment.}
...
%\item The empirical distribution of the random effects often does not resemble the theoretical distribution rendering conventional use of normality tests and Quantile-Quantile plots ineffective for distributional assessment. \todo[inline]{Compared to the other items this one is very specific - it might fit in better as an example, maybe under the first item.}

%\item[Comparison of nested models: ] 

\todo[inline]{What about boundary issues in assessing relevancy of variance components? Comparison of nested models with different fixed effects not possible with REML estimation, too liberal for ML. }

\item[Boundary issues the in comparison of nested models: ] 
\al{When evaluating the significance of terms in the random effects structure, the likelihood ratio test statistic does not have the usual $\chi^2$ reference distribution if we are testing whether a variance component lies on the boundary of the parameter space. \cite{Stram:1994wd} argue that using the usual $\chi^2$ reference distribution leads to conservative results, and suggest using a 50:50 mixture of $\chi^2_q$ and $\chi^2_{q+1}$ when testing $q$ versus $q + 1$ random effects; however, this adjustment is not successful in all cases.}
...
\hh{These issues are well known, and in special circumstances there are fixes for some of them.}
\todo[inline]{ Name some, together with the additional constraints. }
...
\hh{We do not mean this paper to include an exhaustive discussion of all possible tests or testing situations, but rather some suggestions of specific diagnostics to alleviate some of the problems inherent to mixed effects models. The diagnostics in this paper draw from various sources. Some of the diagnostics presented here, are well known diagnostic  tools, such as Q-Q plots or  scatterplots of residuals with trend lines as suggested by \citet{Cook:1999} for ordinary least squares regression. Some diagnostics are suggestions from the literature specific to mixed effects models, such as XXX.
 Some diagnostics are new, addressing practical needs. An example for those are  the cyclone plots of figure~\ref{fig:badcyclone}. The purpose of  these examples is to show some of the variety of graphical diagnostics and emphasize that all of them fit within the graphical inference framework.  We hope to encourage  readers to use their own favorite graphical diagnostics for a lineup of their very own when they next diagnose a model. }


In Section~\ref{sec:select} we focus on model building. In Section~\ref{sec:checking} we present graphical tests for model validation. We also show results of a study investigating the power of three different versions of the familiar Q-Q plot. 
...
%------------------------------------------------------------------------------------
\section{Additional lineups included in the study}\label{app:morelineups}
%------------------------------------------------------------------------------------
