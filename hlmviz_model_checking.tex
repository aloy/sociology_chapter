...
\begin{enumerate}
\item Panels are provided a set of graphs and are asked to pick out the one that does not appear to fit with the rest.  How are these panels selected?  Apparently these observers are “recruited on the web” (p. 23, line 37). How? And are the members of the panel required to have some sense of data (should they be statisticians or have some statistical training)? How does one obtain a sufficient number of observers (as mentioned on p. 8, line 41)? While the ideas presented are nice, more details need to be given on how this would be implemented in practice. Would there be a sufficiently large pool of observers if many researchers submitted their diagnostic plots? And how quickly is the feedback obtained?

{\it all of these are really details and easy to fix by a few extra sentences. XXX the answers are in the appendix/ }


\item  Model selection.  Some discussion of AIC and BIC should be included as these approaches can also be used in model selection.

\item  ``… against the null hypothesis on normality."  Should that be t(3) rather than normality since the random effects (for the 19 additional plots?) were simulated from a t(3) distribution? If “normality” is correct, the text needs to be clearer.

{\it OK, text needs to be clearer. }

\item  It is difficult to believe that the lineup plots help to identify problems with asymptotic approximations and boundary issues as suggested on pages 2-3. Although it is true that the distribution of the error term and random effects may be far from normal the lineup plots do not indicate the deviation of the beta- coefficients estimates from being normally distributed. 

{\it I don't get that statement - I am not sure who misunderstood what, but there is a misunderstanding.}

\item There is indeed a problem of the classical likelihood ratio test on the boundary of variance parameters (page 4), but the authors do not mention the exact F-test which solves this problem (Demidenko E. "Mixed Models: Theory and Applications with R", 2013, page 136).

{\it that might well be, but it does not solve the problem. We need to cite it and clarify that it does not help.}


\item  I do not see how lineup plots help solve the problem of model selection or model checking as described on pages 3-4. How figures presented in the paper are connected to model formulation in equations (1-3)? The authors do not even describe how plots were generated and what parameters were used. How the eye ball assessment of the lineup plots points out to a variable omitted, nonlinearity, or abnormality? Being different is not sufficient, especially in view of the next point.

{\it Reviewer didn't get the point of lineups. }

\item I do not believe that the lineup plots are helpful in identifying outliers or testing the hypothesis at all.  {\it now I don't know which hypothesis the reviewer is talking about }
 Here is why: among any observations there a minimum and there is a maximum. The question “Which of the plots is the most different?” may simply point out to a plot with a maximum (or minimum) expressed feature. But identifying maximum (or minimum) feature in the plot, say, testing for normality of the random as in Figure 9, does not ‘prove’ by any means that the random slope is an outlier. It may be just a cluster with the maximal slope.
...
The purpose of this paper is to illustrate the use of visual inference for diagnosing LME model fits, and propose graphical tests that can be broadly used in situations where the assumptions of conventional inferential procedures are violated. This approach harnesses the power of graphical diagnostics and, with the rigor of the visual inference framework, constructs tests that address some of these problems without requiring additional assumptions. \al{Further, this approach provides a unified testing framework for LME models, even in situations where conventional inference is appropriate.}


The remainder of this paper is organized as follows. 
Section~\ref{sec:vi} provides an introduction to the framework of visual inference and the lineup protocol. Sections~\ref{sec:select} and \ref{sec:checking} expand on the problems encountered in model selection and model checking, respectively, and \al{presents} solutions utilizing visual inference. 
...
It is standard practice to use a $t$-test, $F$-test, or likelihood ratio test to determine whether a fixed effect describes a significant portion of the unexplained variability. \hh{Alternatively, likelihood-based criteria, such as AIC or BIC, are often used to overcome the problem of being only able to deal with nested models.} % in a linear mixed-effects model. 
...
%
%Below is a crude first draft of the table... it only relies on 1000 simulations in each setting. Upping that to 1e4 would be preferable for the final submission in my mind. 
%
%Should there be a d.f. column for the naive p-values?}
%\hhnote{df would be good! I think we also agreed to rename 'cutoff' to 
%'minimal number of records in county'}
\begin{table}[ht]
...
Lineup tests provide us with the framework to test hypotheses and also allow a  subsequent exploration of the plots in the lineup  for additional insight into the data structure. \al{Thus, instead of simply rejecting the null hypothesis, the use of lineups also allows us to explore \emph{why} we are rejecting the null hypothesis}. This approach relies  on the simulation process, the design of the graphics created, and observers, but avoids the reliance on asymptotic reference distributions; thereby circumventing the pitfalls of many commonly used tests. \al{While the value of lineup tests is, perhaps, most obvious in such problematic situations, the approach provides an avenue for testing in all situations, even those in which commonly used tests are appropriate.}
...
\if1\blind
{
R \citep{R} was used to implement all analyses and methods discussed in this paper: lme4 \citep{lme4, lme4b} and HLMdiag \citep{HLMDiag, Loy:JSS} were used for model fitting and  calculation of diagnostics, respectively.  ggplot2 \citep{ggplot2}, nullabor \citep{nullabor}, and gridSVG \citep{gridSVG} provided the basis for the visualizations.
} \fi
...
%------------------------------------------------------------------------------------
\subsection{Experimental Setup and Calculation of $p$-values}\label{sec:pvalues}
\hh{For each of the lineup designs described in the paper we constructed five replicates consisting of the same data plot and different sets of nineteen null plots for a total of 75 different lineups. 
