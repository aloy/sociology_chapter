...
\paragraph{Testing homooscedasticity across groups.}
...
\todo[inline]{I will fill in more here later.}
...
\todo[inline]{Should we introduce the autism data here? or should we have a separate section? It seems like it would be natural to interweave the example with the discussion of visual inference here. For now I will begin to introduce the data here and we can always move it.}

\subsection{Autism data}
%------------------------------------------------------------------------------------
...
\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{ranef3_lineup7.pdf}
	\caption{\label{fig:lineup-ranef3} Lineup testing the need to allow for correlation between the two random effects. These 20 plots compare the predicted random intercept and slope from model XX. Which is the real plot? Note that this is the type of plot that \cite{Morrell:2000ve} discussed. If there were a higher degree of shrinkage in the random effects we would start to see false lines in these plots.}
\end{figure}
...
\missingfigure{Simulate a lineup for the cyclone plots. The autism data has 155 groups, which would be way to many to show in the paper, so we need to reduce the number of groups for the illstration.}
