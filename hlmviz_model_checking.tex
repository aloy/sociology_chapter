...
\end{center}
%
The above factors result in 12 different parameter settings. For each we created two samples, along with two sets of null data each, yielding 48 different lineups. 
Using the Amazon Turk services, 417 participants were each asked to evaluate ten different lineups. This resulted in $n =$ 4,018 responses. We model the probability of selecting the data plot from the lineup, $\pi$, with the help of model $M_1$:
...
\alnote{What significance level was used?}
Out of the 48 non-normal samples, 24 get rejected based on evaluation by observers. Neither of the classical tests comes close to that rejection rate. From left to right, we see the number of rejections from visual inference, Anderson-Darling, Shapiro-Wilks, Cramer-von Mises and Kolmogorov-Smirnov}
\end{table}
