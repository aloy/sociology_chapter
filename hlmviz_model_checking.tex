...
Linear mixed-effects models are versatile models that account for dependence structures when data are organized in groups.
...
\end{equation}
%
where $\bm{y}_i$ is the vector of outcomes for the $n_i$ level-1 units in group $i$, $\bm{X}_i$ and $\bm{Z}_i$ are design matrices for the fixed and random effects, respectively. $\bm{\beta}$ is a vector of $p$ fixed effects governing the global mean structure, $\bm{b}_i$ is a vector of $q$ random effects describing the between-group covariance structure, and $\bm{\varepsilon}_i$ is a vector of level-1 error terms accounting for the within-group covariance structure. We further assume that the random effects,  $\bm{b}_i$,  are a random sample from $\mathcal{N}(\bm{0},\ \bm{D})$ and are independent from the level-1 error terms,  $\bm{\varepsilon}_i$, for which we assume a distribution of $\mathcal{N}(\bm{0},\sigma^2 \bm{I}_{n_i})$. %\bm{R}_i)$. 
Here, $\bm{D}$ is a positive-definite $q \times q$ covariance matrix and $\bm{I}_{n_i}$ denotes the $n_i \times n_i$ identity matrix.
Finally, we assume a covariance of zero for all between group effects.
...
%
\begin{itemize}
\item the \emph{level-1 (observation-level) residuals}, i.e.~the conditional residuals or error terms: $\widehat{\bm{\varepsilon}}_i = \bm{y}_i - \bm{X}_i \widehat{\bm{\beta}} - \bm{Z}_i \widehat{\bm{b}}_i$,

\item and the \emph{level-2 (group-level) residuals}, i.e.~the predicted random effects $\widehat{\bm{b}}_i$
\end{itemize}
%
...
In Sections~\ref{sec:select} and \ref{sec:checking} we focus on the  previously mentioned problems in model selection and model checking, respectively. 
%In Section~\ref{sec:checking} we present graphical tests for model validation. 
Throughout both sections we provide multiple examples of the use of visual inference and draw comparisons with conventional tools. All example data sets are readily available for public use. A short description of each data set can be found in appendix~\ref{sec:datasets}.
...
(see appendix~\ref{sec:pvalues} for a detailed discussion).
