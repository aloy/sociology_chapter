...
\todo[inline]{there's more explanation below - does that help?}
\[
g(\pi_i) = \mu + \tau_{j(i)} +\delta_{k(i)}+ \nu_{s(i)} + u_{u(i)} + d_{d(i)}
\]
where $g$ is the logit link function, \hh{$j(.), k(.), s(.), u(.)$, and $d(.)$ are all indexing functions that relate evaluation $i$ to the corresponding levels in the factor variables, to the observer, or a particular data sample; more specifically, $j(i) \in \{$Control, Standard, Rotated$\}$, $k(i) \in \{2,5,10\}$, $s(i) \in \{20, 30, 50, 75\}$. $u(i)$ maps to the participant id, and $d(i)$ identifies the data sample used in the $i$th evaluation. }
