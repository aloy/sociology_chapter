...
\begin{figure}
	...
	\caption{\label{fig:fanned} \hh{For right now, could you remove axis labels and even scales, such as the numbers along the axis? - We need to take off anything that might identify the data to prevent observers from introducing preconceived biases into their assessment. } Lineup of General Certificate of Secondary Education (GCSE) exam scores at age 16 against standardized London Reading Test scores at age 11. The null plots were created by simulating GCSE exam scores from a random intercept model. Which is the real plot?}
\end{figure}
