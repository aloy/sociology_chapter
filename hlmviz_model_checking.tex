...
\citet[Section 3.5]{Demidenko2013} presents an exact $F$-test in the  null hypothesis $\mathbf{D} = \mathbf{0}$. Unfortunately, the all-or-nothing inclusion of random effects limits the use of this test in practice.
...
In this setting, the null plots are generated using the parametric bootstrap  with a model that omits the variable of interest, \al{details of which can be found in Section~2.2 of the supplement}. The true plot is constructed from the same model but uses the observed data. 
...
Table~\ref{tab:pvalues} shows at the example of the radon data set the huge difference in conventional $p$-values to those obtained through simulation based on 10,000 parametric bootstrap runs. There is a clear discrepancy between the two $p$-values, indicating a need for simulation-based methods. Additionally, the sensitivity of the simulation-based test to the minimum group size is apparent.  

\begin{table}[ht]
\caption{\label{tab:pvalues} A comparison of the naive and the simulation-based version (based on 10,000 parametric bootstrap simulations) of the conventional test by Raudenbush and Bryk of homoscedastic error terms across groups for the radon data. There is a stark discrepancy between the two $p$-values, indicating a need for simulation-based methods. Additionally, the sensitivity of the simulation-based test to the minimum group size is apparent.} 
\centering
...
\hh{The null plots for both of these lineups show Q-Q plots of the predicted random slopes  after re-fitting model (1) to predictions of the radon data with added error terms simulated from a normal distribution. XXXX with sd = ?  }
\hh{In Figure~\ref{fig:qqlineup-1}, the Q-Q plot of the predicted random slopes of model (1) fit to the radon data was inserted into the lineup, while the lineup in Figure~\ref{fig:qqlineup-t} included a Q-Q plot of the random slopes in model (1) where error terms for the re-fit were simulated from a  $t_3$-distribution.}
...
\hh{This is particularly pronounced in this example, because the radon data consists of groups of very uneven sizes, so that there is a high degree of shrinkage.}
...
While the confidence bands show the relationship of the predicted random effects to the hypothesized distribution, this is known to be an ill conceived comparison in many situations. However, the main comparison in lineups is not a comparison of the predicted random effects to the normal distribution, but rather a comparison of the empirical distribution of the random effects between the null and observed plots. 
...
Additionally, this example shows \hh{the impact of confounding between the different levels of the mixed effects model on the distribution of the predicted random effects \citep{adam}: even for null plots, where error terms were generated from a normal distribution before re-fitting the model, predicted random effects do not look normal}---in fact, 16 of the null plots fail the Anderson-Darling test of normality at a significance level of 0.05.  
...
\hh{demonstrate that} lineups \hh{are indeed able to} detect distributional violations we constructed another lineup where the ``true plot'' was \hh{based on} random effects simulated from a $t_3$ distribution---i.e.\ using model~(\ref{eq:hlm}), error terms $\varepsilon \sim t_3$ were simulated and the model was re-fit, deriving a new set of residuals. For the null plots, error terms were simulated from a normal distribution and the model was re-fit to derive the corresponding residuals. 
 ...
 \hh{that we can visually detect a violation of normality in the error terms in the distribution of residuals in Figure~\ref{fig:qqlineup-t}. }
