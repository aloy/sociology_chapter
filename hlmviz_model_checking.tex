...
Data for the null plots were generated from a parametric bootstrap \al{as outlined in Section XX of the supplment.
...
Residual scatterplots are useful in checking that level-1 residuals are homoscedastic with respect to the explanatory variables, but they do not possess the same power as box plots when they investigate potential differences in variability between groups. 
...
\al{bootstrap observations generated using the parametric bootstrap as outline in section XX of the supplement. Consequently, the null plots represent Q-Q plots of the random slopes for a properly specified model.}
...
All of the lineups presented in this paper use a parametric bootstrap to generate plots consistent with the null hypothesis. This section outlines the parametric bootstrap for LME models and provides more detail about how the null plots for each situation were generated.
...
\item Generate a bootstrap sample $\bm{y}_i^*$ for each group $i=1,\ldots,g$ from $\bm{y}_i^* = \bm{X}_i \widehat{\beta} + \bm{Z}_i \bm{b}_i^* + \bm{\varepsilon}^*_i$.
\item Refit the model to the bootstrap samples.
\item Repeat steps 1--4 $B$ times.
\end{enumerate}

We detail the models suggested in the paper for use in model selection and model checking below. The parametric bootstrap was used to generate the null plots in each situation. During model selection the simpler models are used to generate the null plots from the parametric bootstrap, while model checking bootstraps the original model.
...
%
\[
\bm{y}_i = \bm{X}_i^{(c)} \beta + \bm{Z}_i \bm{b}_i + \bm{\varepsilon}_i
\]
%
