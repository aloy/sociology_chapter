...
It is standard practice to use a $t$-test, $F$-test, or likelihood ratio test to determine whether a fixed effect describes a significant portion of the unexplained variability. \hh{Often, likelihood-based criteria, such as AIC or BIC, are used to overcome the problem of being only able to deal with nested models.} % in a linear mixed-effects model. 
...
To determine whether lineups can detect distributional violations we constructed another lineup where the ``true plot'' was constructed from random effects simulated from a $t_3$ distribution\hh{, i.e.\ based on model~(\ref{eq:hlm}), error terms $\varepsilon \sim t_3$ are simulated and the model is re-fit, deriving a new set of residuals. For the null plots, error terms are simulated from a normal distribution and the model is re-fit to derive the corresponding residuals. }
...
Plot design is important---some designs are better for revealing anomalies than others. At a population level, the results obtained from the same plot design are very stable. %This is reassuring and allows us to introduce the concept of power of tests into visual inference. 
