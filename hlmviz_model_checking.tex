...
\begin{figure}
	...
	\caption{\label{fig:fanned} Which plot is the most different from the other plots? What makes your choice different from the other plots?
...
\alnote{I went through and tried to merge everything today. This is only my first attempt at the merge. I am now rereading this section trying to work out the kinks.}
...
\paragraph{Fixed effects.} To test the significance of a fixed effect, we suggest using a plot comparing a residual quantity from the model omitting the variable of interest and the variable of interest. The residual that should be used depends on the level at which the variable of interest enters the model---if the variable enters at the observation-level (level-1), then the level-1 residuals should be used; if the variable enters at the group level, then both the level-1 and level-2 residuals should be explored as additional variation at either level could be explained by this variable. Additionally, the type of plot depends on the variable type: if a continuous variable is targeted,  a scatterplot with a smoother is suitable for testing; for a discrete covariate, we can make use of side-by-side boxplots. 


In this setting, the null plots are generated using the parametric bootstrap \todo{does the bootstrap need a citation?} with a model that omits the variable of interest. The true plot is constructed from the same model but using the observed data. Figure~\ref{fig:boxplot-ordered} illustrates the use of this type of lineup to determine whether a child's language development at age 2 (low, medium, or high) helps explain the development of social skills from childhood to adolescence for children diagnosed with autism spectrum disorder. Which is the real plot? 
...
\alnote{How much more should we say here? I feel like this discussion ends a little abruptly, but I don't want to ramble either.}
...
The lineup in Figure~\ref{fig:constvar1} consists of 20 plots of the level-1 residual against (standardized LRT scores)$^3$, one of the explanatory variables included inthe model. If any panel of this lineup is considered separately, an analyst may come to the conclusion that the level-1 residual variance decreases as (standardized LRT scores)$^3$ increases; however, inserting the true plot into the lineup forces the analyst to consider the features such a plot will exhibit under the null hypothesis of homogenous variance. Based on the lineup, is there a violation of this model assumption? If we identify the true plot (panel $2^3-3$) this provides evidence of a violation, but if we are unable to this provide evidence that any structure observed in the plot is artifical.
...
\end{equation}
%
which has an approximate $\chi^2_{m^*-1}$ reference distribution when the data are normal and the group sizes are ``large enough''. Here we use $m^*$ because ``small'' groups may be excluded from the calculation as small group sizes provide less reliable information about the residual variance, but this is a subjective choice. If the distributional assumptions are violated or we do not have large enough group sizes, then the approximation to the $\chi^2$ distribution breaks down. In the Methylprednisolone study each subject was observed at most 5 times, with 19 subjects dropping out of the study early. Due to the small group sizes, the $\chi^2$ approximation would be inappropriate, forcing the analyst to rely on simulation to construct the sampling distribution of the test statistic, which is computationally more demanding than the generation of 19 null plots.
...
To check the assumption of linearity we suggest the use of scatterplots and boxplots of  both the residuals (either level-1 or -2) against the fitted values or an explanatory variable. If the assumption of linearity is upheld, then the residuals plots should be centered around zero and exhibit ``random scatter.'' As illustrated in the previous section, such plots are also used to check for homogeneity of variance.
...
%------------------------------------------------------------------------------------
\todo[inline]{this section should be folded into the previous section on the Q-Q plot discussion }
...
\alnote{I think the envelope that used was not a simulation envelope, but rather a point-wise confidence envelope based on the order statistics for an independent sample from the normal distribution.}
...
\end{figure}
 ...
 $X_i$, with $X_i \sim B_{1, \pi_i}, 1 \le i \le n$,  is the binary decision on the $i$th evaluation and $\pi_i$ is the probability with which the observer chooses the data plot. This probability is influenced by a number of factors:
...
\hh{Note that none of the data plots in the lineups were actually created using data from a normal distribution. This should lead to rejection of the null hypothesis in every single instance. This is not quite true, as can be seen in table~\ref{tab:reject}. But what becomes also evident is the high power  visual inference has. Based on lineups we are able to reject non-normality much more  often than based on any of the classical tests.}
...
Result & Visual & AD & SW & CVM & KS \\ 
  ...
  not reject & 24 & 38 & 32 & 40 & 38 \\ 
   \hline
\end{tabular}
\caption{\label{tab:reject} Out of the 48 non-normal samples, 24 get rejected based on evaluation by observers. Neither of the classical tests comes close to that rejection rate. From left to right, we see the number of rejections from visual inference, Anderson-Darling, Shapiro-Wilks, Cramer-von Mises and Kolmogorov-Smirnov}
\end{table}
