...
\hh{Various approaches to deal with this problem have been suggested in the literature: }
\al{\citet[Section 3.5]{Demidenko2013} presents an exact $F$-test in the  null hypothesis $\mathbf{D} = \mathbf{0}$.} \hh{Unfortunately, the all-or-nothing inclusion of random effectslimits the use of this test in practice.}
...
\begin{table}[ht]
\caption{\label{tab:pvalues} A comparison of the naive and the simulation-based version (based on 10,000 simulated statistics \hh{XXX teensy bit more detail on the simulation}) of the conventional test \hh{by Raudenbush and Bryk} of homoscedastic error terms across groups for the radon data. There is a clear discrepancy between the two $p$-values, indicating a need for simulation-based methods. Additionally, the sensitivity of the simulation-based test to the minimum group size is apparent.} 
\centering
