...
with 40.3\% pointing to the trend as distinguishing feature. 
...
%Note that with a higher amount of shrinkage in the random effects we start to see more lines with a higher amount of slope. 
 ...
 \hh{The lineup allows us to gauge the amount of correlation between the random effects while accounting for the effect of shrinkage in the model,} \al{avoiding the over-interpretation of structure in such plots discussed by \cite{Morrell:2000ve}.}
  In Figure~\ref{fig:ranef-corr} the true plot (panel~$10 + \sqrt{25}$) was identified \al{by 41 of the 69 observers}, providing evidence supporting the inclusion of the additional parameter.
...
\multicolumn{5}{l}{Signif. codes:  0 Ô***Õ 0.001 Ô**Õ 0.01 Ô*Õ 0.05 Ô.Õ 0.1 Ô Õ 1}
\end{tabular}
\caption{\label{tab:results} Overview of all lineup evaluations. Shown are ratios of number correct compared to number of total evaluations. \hh{maybe we should include a star system for significances}} 
\alnote{Was examuncorr actually the plots for investigating homogeneous level-1 variance?}
\alnote{Were there 29 or 23 Turkers in Lendie's study? I have 23 in the body of the paper, but certainly could have transcribed that incorrectly.} \hh{23 is right}
\end{table}
