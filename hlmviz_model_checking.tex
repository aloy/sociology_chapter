...
Valid model-based statistical inference relies on proper specification of a model, making diagnostic tools central to the analytic process. Graphical methods are commonly used to check the assumptions of a model. \hh{Depending on methodology, different diagnostics are suggested for assessing distributional assumptions and assessing model fits in detail. In the linear model setting, Quantile-Quantile (Q-Q) plots and scatterplots with smoothers are often used \citep{Cook:1999}.}  \al{In the Bayesian framework, \citet{Gelman:2004gg} proposes simulation from posteriors to assess goodness-of-fit.  }
...
At the core of this criticism is the fact that decisions are being made based on a single plot. When a single plot is used, the detection of any unexpected structure leads to the conclusion that the model is inappropriate; however, \hh{complex models introduce structures  that are  observed,} for example, 
...
\alnote{Should we spell out the ``linear mixed-effects model'' each time, or use the abbreviation? I think LME is well-known in the community, so it shouldn't add undue burden on the reader, but that is just my opinion.} \hhnote{LME is fine with me}
...
In Section~\ref{sec:select} we focus on model building. In Section~\ref{sec:checking} we present graphical tests for model validation. 
...
\alnote{I need to go back an really emphasize the lack of effective conventional tests here}
...
\begin{figure}
	...
	\caption{\label{fig:qqlineup-t} Lineup  testing for normality of the random slope for the radon data. Which of the plots is the most different? Which feature led you to your choice?}
\end{figure}
