...
due to the breakdown of asymptotic results, boundary issues, and our ability to see patterns
in diagnostic plots where none exist. While these issues are well known and adjustments
have been proposed, they require one to keep track of all the special circumstances that
may arise. In this paper we illustrate the use of visual inference for diagnosing LME 
model fits and propose graphical tests that that can be broadly used in situations where 
the assumptions of conventional inferential procedures are violated. This approach
provides a unified framework for diagnosing LME model fits and does not require that one remember different rules of thumb for different circumstances.
\keywords{Statistical graphics; Lineup protocol; Visual tests; Model diagnostics;  ...}
\alnote{We also need to decide on keywords}
\end{abstract}
...
\alnote{One comment I could see coming back is the fact that LME models allow for more general covariance structures than we specify above. Using $\sigma^2 \bm{R}$ would be a common general formulation, where $\bm{R}$ must be positive definite. We could use the general form and then state we use examples with the similar form. I don't know if this is a big deal at all, I was just trying to think critically.}
\hhnote{Let's go with the positive definite $R$, it's not going to change anything in the type of charts we are looking at ... }
