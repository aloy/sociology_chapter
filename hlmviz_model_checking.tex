...
\al{
Throughout this paper we discuss visual inference in the framework of hierarchical linear models and suggest a graphical tests in situations where the assumptions of conventional inference are violated. In Section~\ref{sec:select} we focus on model building. Section~\ref{sec:checking} focuses on model validation and presents the results of a study investigating the power of three different versions of a Quantile-Quantile plot. In Section~\ref{sec:conclusion} we provide concluding remarks.
%In sections~\ref{sec:select} and \ref{sec:checking} we discuss how visual inference can be utilized to select and validate hierarchical linear models. 
%While we focus on examples in which we encounter problems with conventional inference, visual inference can be applied more generally...
}
%In the next section we discuss how visual inference can be utilized to explore hierarchical linear models.
...
%%------------------------------------------------------------------------------------
%\section{Visual inference for hierarchical linear models}\label{sec:hlms}
%%------------------------------------------------------------------------------------
...
%%------------------------------------------------------------------------------------
%
%\todo[inline]{Let's move the notation into the introduction}
%
%In this paper we consider the continuous response hierarchical linear model
%%
%\begin{equation}\label{eq:hlm}
...
In this setting, the null plots are generated using the parametric bootstrap \todo[inline]{does the bootstrap need a citation?} with a model that omits the variable of interest. The true plot is constructed from the same model but using the observed data. 
...
%------------------------------------------------------------------------------------
Quantile-Quantile (Q-Q) plots \citep{Wilk:1968} are often used as an informal check of the distributional assumptions made on the level-1 and -2 residuals; however, an interrelationship between the residual quantities exists in hierarchical linear models that can render such an assessment inappropriate. Additionally, strong assumptions are required for the empirical distributions of the residuals in the hierarchical linear model to converge in probability to their true distributions \citep[Theorem 3.2 and Lemma 3.1]{Jiang:1998vt}. If these assumptions are not satisfied, then the empirical distribution of the residuals may not resemble the hypothesized distribution under a properly specified model. Consequently, single Q-Q plots will often lead to erroneous conclusions about the distributional assumptions which can be overcome, at least partially, using lineups.
...
\al{To further support the assessment of normality using lineups a study was conducted to compare three different versions of a Q-Q plot.
%We are testing three different versions of a Q-Q plot, 
The three versions are displayed in figure~\ref{qqplots} and include (from left to right): a standard Q-Q plot, a Q-Q plot with an added 95\% pointwise confidence region based on the estimated standard error of the order statistics for an independent sample from the theoretical distribution,} \todo{I didn't find a good general reference, but this is implemented all over the place.}
\al{ and a rotated (i.e., detrended) Q-Q plot of the same data. To explore the results of this study we must first define some additional notation.}
...
&  $\delta$ \ \ \ degrees of freedom (2, 5, 10) \al{for the $t$ distribution} and \\
&  $\nu$  \ \ \ sample size (10, 20, 50, 75), \\
$\ell$ &  the level of difficulty based on the actual sample, and \\
...
Using the Amazon Turk \todo{Mechanical Turk?} services, 417 participants were each asked to evaluate ten different lineups. 
\alnote{Why $n =$ 4,018? Did you throw out responses? Is there missing data?}
This resulted in $n =$ 4,018 responses. We model the probability of selecting the data plot from the lineup, $\pi$, with the help of model $M_1$:
